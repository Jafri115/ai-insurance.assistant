...
Supports LLaMA, Mistral, Gemma, Phi-3 and other transformer models.
"""
...
class InsuranceTrainer:
    ...
    def setup_model_and_tokenizer(self):
        ...
        if use_4bit:
            ...
            reason = []
            if is_windows:
                reason.append("Windows")
            if not has_cuda:
                reason.append("no CUDA GPU")
            if not bnb_available:
                reason.append("bitsandbytes not installed")
        ...
        if not has_cuda:
            # CPU-friendly settings: lower peak RAM and avoid flash-attn paths
            model_kwargs.update({
                "low_cpu_mem_usage": True,
                "attn_implementation": "eager",
                # Stream weights from disk to reduce peak memory during load
                "offload_state_dict": True,
                "offload_folder": os.path.join(str(self.training_args.output_dir), "offload"),
            })
        ...
        # Setup LoRA - auto-detect target modules when requested
    ...
    def load_dataset(self):
        ...
        def load_json_dataset(file_path: str) -> Optional[Dataset]:
                ...
                sanitized.append(
                    {
                        "instruction": instr
                        if instr
                        else "Answer the insurance-related question accurately and helpfully.",
                        "input": inp,
                        "output": out,
                    }
                )
        ...
        self.train_dataset = self.train_dataset.map(
            self._preprocess_function, batched=False, remove_columns=self.train_dataset.column_names
        )

        if self.eval_dataset:
            self.eval_dataset = self.eval_dataset.map(
                self._preprocess_function, batched=False, remove_columns=self.eval_dataset.column_names
            )
...
def main():
    ...
    # Model arguments
    parser.add_argument(
        "--model_name", type=str, default="microsoft/Phi-3-mini-4k-instruct", help="Model name or path"
    )
    parser.add_argument(
        "--trust_remote_code", action="store_true", help="Trust remote code when loading model/tokenizer"
    )
    parser.add_argument("--dataset_path", type=str, default="data/processed/train.json", help="Training dataset path")
    parser.add_argument(
        "--validation_path", type=str, default="data/processed/validation.json", help="Validation dataset path"
    )
    parser.add_argument(
        "--output_dir", type=str, default="models/insurance-assistant", help="Output directory"
    )
    ...
    parser.add_argument(
        "--optim", type=str, default=None, help="Override optimizer (e.g., adamw_torch, paged_adamw_8bit)"
    )
    parser.add_argument(
        "--grad_accumulation", type=int, default=1, help="Gradient accumulation steps to simulate a larger batch"
    )
    ...
    parser.add_argument(
        "--lora_target_modules", type=str, default="auto", help="Comma-separated target modules or 'auto' to detect"
    )
    ...
    model_args = ModelArguments(
        ...
        trust_remote_code=bool(args.trust_remote_code) or True,
    )
    data_args = DataArguments(
        dataset_path=args.dataset_path, validation_path=args.validation_path, max_length=args.max_length
    )
    lora_args = LoRAArguments(
        lora_r=args.lora_r, lora_alpha=args.lora_alpha, lora_dropout=args.lora_dropout
    )

    # Pick optimizer with safe fallback when bitsandbytes is unavailable
    use_bnb = False
    try:
    ...
    chosen_optim = args.optim if args.optim else ("paged_adamw_8bit" if use_bnb else "adamw_torch")
